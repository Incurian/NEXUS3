...
- [x] Wire schema validation into ingress paths.
  ...
  - [x] M1 Phase 2 protocol error-object strictness slice: `RpcResponseEnvelopeSchema.error` now enforces typed `RpcErrorObjectSchema` shape (including strict nested field/extra validation), `parse_response` preserves legacy wording for non-object/missing required `error` fields, and successful parsed error payloads are returned as plain dicts without default `data: null`.
  ...
  - [x] M1 Phase 2 projection cleanup slice: removed temporary schema-field projection branches in `rpc/dispatcher.py::_handle_send` and `rpc/global_dispatcher.py::_handle_create_agent`; ingress now validates full param objects while preserving legacy error wording for malformed known fields.
