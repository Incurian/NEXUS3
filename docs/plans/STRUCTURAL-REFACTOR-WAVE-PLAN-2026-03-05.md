...
- 2026-03-09: Phase 3D (Pool lifecycle extraction) completed in WSL.
  ...
  - Focused validation passed:
    ...
    - `.venv/bin/pytest -q tests/integration/test_permission_inheritance.py tests/integration/test_sandboxed_parent_send.py` (`51 passed`)
- 2026-03-09: Wrapper-cleanup follow-on + Session wrapper-retirement closeout
  completed in WSL.
  ...
  - Remaining Session wrappers `_execute_single_tool(...)`,
    `_handle_mcp_permissions(...)`, `_handle_gitlab_permissions(...)`,
    `_execute_skill(...)`, and `_execute_tools_parallel(...)` were removed.
...
- Next gate: deferred structural tracker/docs sync closeout is complete in this
  status wave; next queued execution task is provider keep-alive real-endpoint
  evidence capture.
...
- [x] Remove remaining temporary compatibility wrappers after one full green cycle.
- [x] Update deferred tracker rows for structural refactors and display config.
