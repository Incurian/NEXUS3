...
5. Structural refactor wave:
   ...
   - Status note (2026-03-09): kickoff + REPL + Session Phase 2A/2B/2C/2D/2E/2F/2G/2H
     ...
     - Phase 1B completed: runtime/client-discovery helpers extracted to
       ...
       `nexus3/cli/repl.py`.
     - Phase 2A completed: Session compaction provider/summary internals
       extracted to `nexus3/session/compaction_runtime.py` with
       `Session._get_compaction_provider(...)` and
       `Session._generate_summary(...)` retained as compatibility wrappers.
     - Phase 2B completed: Session tool execution primitives extracted to
     ...
     - Phase 2E completed: Session streaming callback-adapter internals
       ...
       wrapper.
     - Phase 2F completed: Session tool-loop event internals extracted to
     ...
     - Phase 2H completed: Session non-tool simple streaming internals
     ...
     - Phase 3B completed: pool create-path authorization/locking helpers
       extracted to `nexus3/rpc/pool_create.py` with `create(...)`,
       `create_temp(...)`, and create-authorization enforcement now delegating
       to extracted helpers.
     - Phase 3C completed: pool restore-path internals extracted to
