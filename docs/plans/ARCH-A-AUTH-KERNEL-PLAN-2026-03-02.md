...
- Progress (2026-03-05): M3 next slice removed remaining non-kernel destroy bypasses in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) by routing external (`requester_id=None`) and `admin_override=True` destroy authorization through kernel adapter evaluation, preserving self/parent allow semantics and deny wording while adding focused external/admin allow + fail-closed deny coverage in [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py).
- Progress (2026-03-05): M3 next slice made tool-enabled authorization in [session/enforcer.py](/home/inc/repos/NEXUS3/nexus3/session/enforcer.py) (`_check_enabled`) kernel-authoritative via adapter path, preserved exact deny wording (`Tool '<tool>' is disabled by permission policy`), preserved `check_all` ordering precedence over action-allowance denial, and added focused coverage in [tests/unit/session/test_enforcer.py](/home/inc/repos/NEXUS3/tests/unit/session/test_enforcer.py).
