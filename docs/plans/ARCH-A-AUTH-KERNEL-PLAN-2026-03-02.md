...
- [x] Implement adapters for session and RPC call sites.
...
- Progress (2026-03-06): M3 Commit 18 made create authorization in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) kernel-authoritative, removed dead create shadow-compare mismatch logging branches, preserved existing max-depth/base-ceiling/delta-ceiling `PermissionError` wording, and updated authoritative coverage in [tests/unit/rpc/test_pool_create_auth_shadow.py](/home/inc/repos/NEXUS3/tests/unit/rpc/test_pool_create_auth_shadow.py) and [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py) (fail-closed kernel deny + requester/parent mismatch enforcement).
- Progress (2026-03-06): M3 next slice removed remaining non-kernel destroy bypasses in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) by routing external (`requester_id=None`) and `admin_override=True` destroy authorization through kernel adapter evaluation, preserving self/parent allow semantics and deny wording while adding focused external/admin allow + fail-closed deny coverage in [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py).
- Progress (2026-03-06): M3 next slice made tool-enabled authorization in [session/enforcer.py](/home/inc/repos/NEXUS3/nexus3/session/enforcer.py) (`_check_enabled`) kernel-authoritative via adapter path, preserved exact deny wording (`Tool '<tool>' is disabled by permission policy`), preserved `check_all` ordering precedence over action-allowance denial, and added focused coverage in [tests/unit/session/test_enforcer.py](/home/inc/repos/NEXUS3/tests/unit/session/test_enforcer.py).
- Progress (2026-03-06): M3 next slice kernelized session-level MCP/GitLab level gates in [session/session.py](/home/inc/repos/NEXUS3/nexus3/session/session.py) via dedicated `TOOL_EXECUTE` adapters while preserving legacy deny wording and confirmation behavior, and removed the remaining pre-kernel unknown-target bypass in [session/enforcer.py](/home/inc/repos/NEXUS3/nexus3/session/enforcer.py).
- Progress (2026-03-06): M3 next slice added unconditional create lifecycle-entry authorization in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) so root and nested create flows always traverse kernel authorization (`check_stage="lifecycle_entry"`), with focused root-path traversal and fail-closed deny coverage in [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py) and [tests/unit/rpc/test_pool_create_auth_shadow.py](/home/inc/repos/NEXUS3/tests/unit/rpc/test_pool_create_auth_shadow.py).
- Progress (2026-03-05): M3 hardening slice resolved parent permission ceilings from live parent agent state in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) for parented creates, fail-closed when `parent_agent_id` does not resolve, ignored forged `config.parent_permissions` mismatches with warning telemetry, and added focused coverage in [tests/unit/rpc/test_pool_create_auth_shadow.py](/home/inc/repos/NEXUS3/tests/unit/rpc/test_pool_create_auth_shadow.py).
- Progress (2026-03-05): M3 hardening slice kernelized MCP tool-visibility gating in [rpc/pool.py](/home/inc/repos/NEXUS3/nexus3/rpc/pool.py) for both create and restore flows via `TOOL_EXECUTE` adapter checks, preserving existing MCP level behavior while centralizing visibility decisions through pool-local authorization kernel evaluation, with focused coverage in [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py).
- Progress (2026-03-05): M3 decision checkpoint deferred full migration of create-stage `parent_can_grant` computation into adapter internals because `AuthorizationRequest.context` is intentionally scalar-typed and adapter-local `can_grant()` would require broader authorization-request model redesign; recorded current boundary as intentional (`pool` computes `parent_can_grant`, adapter consumes stage result) and added stage-order regression coverage in [tests/unit/test_pool.py](/home/inc/repos/NEXUS3/tests/unit/test_pool.py).
- Progress (2026-03-05): M3 hardening slice kernelized `PermissionEnforcer` path allow/deny enforcement in [session/enforcer.py](/home/inc/repos/NEXUS3/nexus3/session/enforcer.py) via `TOOL_EXECUTE`/`PATH` adapter decisions while preserving existing `PathDecisionEngine` semantics and deny wording for legacy path-denied cases, with focused authoritative-path coverage in [tests/unit/session/test_enforcer.py](/home/inc/repos/NEXUS3/tests/unit/session/test_enforcer.py).
- Progress (2026-03-05): M3 cleanup slice removed the duplicate local create ceiling precheck from [skill/builtin/nexus_create.py](/home/inc/repos/NEXUS3/nexus3/skill/builtin/nexus_create.py) so `nexus_create` now defers parent-ceiling authorization entirely to RPC/pool kernel flow while preserving downstream create error text, with focused coverage in [tests/unit/skill/test_nexus_create.py](/home/inc/repos/NEXUS3/tests/unit/skill/test_nexus_create.py).
- Progress (2026-03-05): M3 closeout slice kernelized live REPL permission mutations in [cli/repl_commands.py](/home/inc/repos/NEXUS3/nexus3/cli/repl_commands.py) for preset changes and tool enable/disable checks, preserving existing ceiling-deny wording while fixing preset-switch state retention (`depth`, `session_allowances`, sandbox `cwd`) and adding focused coverage in [tests/unit/test_repl_commands.py](/home/inc/repos/NEXUS3/tests/unit/test_repl_commands.py).
