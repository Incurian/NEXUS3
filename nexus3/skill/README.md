...
| `patch` | Apply unified diffs with validation (exact-path target preference, ambiguity fail-closed) | `target`, `diff?`, `diff_file?`, `mode?`, `fidelity_mode? (default: byte_strict)`, `fuzzy_threshold?`, `dry_run?` |
