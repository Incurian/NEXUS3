...
| `session.py` split (~1100 lines) | completed extraction + wrapper retirement on `feat/arch-overhaul-execution` (remaining Session wrappers removed in closeout wave) | M |
| `pool.py` split (~1250 lines) | completed extraction + wrapper retirement on `feat/arch-overhaul-execution` | M |
