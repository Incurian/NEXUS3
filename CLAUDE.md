...
- This file mirrors the stable project guidance only.
- For the latest branch status and handoff, see
  [AGENTS.md `## Current Handoff`](/home/inc/repos/NEXUS3/AGENTS.md#current-handoff).

Claude Code skills:

- `.claude/skills/nexus/SKILL.md`: REPL/server startup modes
- `.claude/skills/nexus-rpc/SKILL.md`: RPC command guidance
...
Keep `AGENTS.md` and `CLAUDE.md` consistent when behavior or workflow guidance
changes.

## Reference Docs

Detailed references live in companion files:
...
- `AGENTS.md` owns the live handoff model.
- Resume from
  [AGENTS.md `## Current Handoff`](/home/inc/repos/NEXUS3/AGENTS.md#current-handoff)
  first after disconnects or compaction.
- Keep `CLAUDE.md` as the stable mirror, not the running history log.
...
Known Codex CLI sandbox caveats:

- sandboxed test runs may hang on some `asyncio.to_thread(...)` file-I/O
  paths
- sandbox networking blocks `git push`
- when the user explicitly asks to push, use the unsandboxed or escalated push
  path immediately

Live validation remains required for changes affecting agent behavior, RPC,
...
- Update `AGENTS.md` and `CLAUDE.md` when stable workflow guidance changes
...
For the latest active slice, validation state, and next gate, see
[AGENTS.md `## Current Handoff`](/home/inc/repos/NEXUS3/AGENTS.md#current-handoff).

## Known Failures

None currently tracked in this trimmed mirror.

If a temporary breakage needs to be carried forward, record it here and link
the active investigation from
[AGENTS.md `## Current Handoff`](/home/inc/repos/NEXUS3/AGENTS.md#current-handoff).
