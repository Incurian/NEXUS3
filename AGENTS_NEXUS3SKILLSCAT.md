...
| `outline` | `path`, `file_type`?, `language`?, `depth`?, `preview`?, `signatures`?, `line_numbers`?, `tokens`?, `symbol`?, `diff`? | Structural outline of file/directory; directory mode is non-recursive, `symbol` returns a source excerpt for files only, `file_type`/`language` can override parser detection for files, and ambiguous symbol matches fail closed |
...
| `nexus_create` | `agent_id`, `preset`?, `disable_tools`?, `cwd`?, `model`?, `initial_message`?, `wait_for_initial_response`? | Create agent (initial message queued by default) |
